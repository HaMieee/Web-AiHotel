...
const TableThree: React.FC<ITableManage> = ({
    ...
    const renderTagTd = (value, key, idx) => {
        if (key === 'id') {
            return <TableCell style={{width: '50px'}} align="center" key={idx}>{idx}</TableCell>
        ...
        return <TableCell
            style={{
                ...
                whiteSpace: 'nowrap'
        }} align="center" key={`${key}_${idx}`}>{value}</TableCell>
        ...
        <TableContainer component={Paper} sx={{ maxHeight: 440 }}>
            ...
            >
                ...
                <TableHead>
                    <TableRow>
                        {columns.map((column, c_index) => (
                            <TableCell align={"center"} key={c_index}>{column}</TableCell>
                        ))}
                    </TableRow>
                </TableHead>
                ...
                <TableBody>
                    ...
                    {!isEmpty(rows) && map(rows, (row, i_index) => (
                        <TableRow
                            hover
                            key={i_index}
                        ...
                        >
                            ...
                            {actions && !isEmpty(actions) && <TableCell style={{textAlign: 'center'}}>{renderActions(row.id, row.status)}</TableCell>}
                        </TableRow>
                    ))}
