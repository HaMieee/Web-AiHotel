...
const CreateHotelModal: React.FC<ICreateHotelModal> = ({
    ...
    useEffect(() => {
        if (isShow) {
            if (action === 'update' && !isEmpty(hotelDetail)) {
                const roomTypeIds = hotelDetail?.room_types?.map(type => type.id).filter(id => id !== undefined) as number[];
                setFormCreateHotel({
    ...
    const handleSelectRoomTypes = (recordId: number | undefined) => {
        if (recordId) {
            if (!isEmpty(formCreateHotel.room_types) && formCreateHotel.room_types) {
                if (formCreateHotel.room_types.includes(recordId)) {
                    setFormCreateHotel({
                        ...formCreateHotel,
                        room_types: formCreateHotel.room_types.filter(id => id !== recordId)
                    })
                } else {
                    setFormCreateHotel({
                        ...formCreateHotel,
    ...
    return (
        <>
            <Dialog
                ...
                maxWidth={'md'}
            >
                <DialogTitle>{action === 'create' ? 'Thêm khách sạn mới' : 'Chỉnh sửa khách sạn'}</DialogTitle>
                <DialogContent>
                    <Box
                        component="form"
                        sx={{
                        ...
                        <Grid container spacing={2}>
                            {roomTypesData.map((type) => (
                                <Grid item xs={3} key={type.id}>
                                    <FormControlLabel
                                        control={<Checkbox checked={formCreateHotel.room_types?.includes(type.id || 0)} />}
                                        label={type?.name}
                ...
                <DialogActions>
                    <Button variant="text" color={'inherit'} onClick={handleClose}>Đóng</Button>
                    {action === 'create' ? <Button variant="contained" color={'primary'} onClick={handleCreateHotel}>Tạo mới</Button> : ''}
                    {action === 'update' ? <Button variant="contained" color={'warning'} onClick={handleUpdateHotel}>Lưu</Button> : ''}
                </DialogActions>
            </Dialog>
        </>
    )
