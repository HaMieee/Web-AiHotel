...
const sendMail = async (email: string) => {
  try {
    const response = await axiosInstance.post('/api/auth/send-reset-password-email', {email: email})
    ...
    return(
        ...
        <div className="wrapper_resetPassword">
            <div className="container_resetPassword">
              ...
              <p className="text-center">Nhập email của bạn để thiết lập mật khẩu của bạn </p>
              <div className="top">
              <input type="text" placeholder="Email của bạn"  value={email} onChange={e => setEmail(e.target.value)}/>
              </div>
              <div className='reset'>
                <button type="submit" className='confirm' onClick={handleSubmit} >Xác nhận</button>
                <button type="submit" className='reject'onClick={() => navigate('/login')} >Hủy bỏ</button>
              </div>
            </div>
          </div>
        </>
