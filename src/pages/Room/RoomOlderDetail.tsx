...
const RoomOlderDetail = () => {
    const reservationsState = useSelector((state: RootState) => state.auth.userInfo.reservations);
    const { id } = useParams();
    ...
    useEffect(() => {
        if (!isEmpty(reservationsState)) {
            const reserDetail = reservationsState?.find(reser => reser.id === Number(id));            
            if (reserDetail) {
                setReservationDetail(reserDetail);
                setIsLoading(false)
            }
    ...
    return(
        <div className="roomOlderDetail">
            <div style={{fontSize:'54px', fontStyle:'italic'}}>Xin chào {reservationDetail.user?.name}</div>
            ...
            <div>
                <Button variant="contained" onClick={() => setIsShow(true)}>Hủy đặt phòng
                </Button>
                <Button variant="outlined" style={{marginLeft:'12px'}} onClick={() => navigate('/')} >Đóng</Button>
            </div>
